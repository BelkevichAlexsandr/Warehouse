...
from app.schemas.manufacturer import ManufacturerFullModel, ManufacturerModel, PatchManufacturerModel
...
@router.get("/", status_code=status.HTTP_200_OK, response_model=list[ManufacturerFullModel])
async def get_manufacturers(
    search: str | None = Query("", max_length=100),
    need_id: Annotated[list[int], Query()] = None,
    db: AsyncSession = Depends(get_db),
):
    return await ManufacturerService(db=db).get_all_manufacturers(search=search, need_id=need_id)


@router.get("/{item_id}/", status_code=status.HTTP_200_OK, response_model=ManufacturerFullModel)
...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model=ManufacturerFullModel)
...
async def patch_manufacturer(
    ...
    return await ManufacturerService(db=db).update_manufacturer(item_id=item_id, request=request)
